...
import com.arcussmarthome.common.scene.Scene;
...
import com.arcussmarthome.core.platform.handlers.AddTagsModelRequestHandler;
import com.arcussmarthome.core.platform.handlers.GetAttributesModelRequestHandler;
import com.arcussmarthome.core.platform.handlers.RemoveTagsModelRequestHandler;
...
import com.arcussmarthome.platform.rule.analytics.SceneAnalyticsWrapper;
...
public class SceneHandler implements PlaceEventHandler {
   ...
   protected void onDeleted(String address) {
      ...
      for(int i = 0; i < actions.size(); i++) {
         com.arcussmarthome.messages.type.Action action = new com.arcussmarthome.messages.type.Action(actions.get(i));
      ...
      if(requiresRebuild(attributes)) {
         ...
         if(actions != null && actions.size() > 0) {     
	         ...
	         for(Map<String, Object> curAction : actions) {	        	 
	        	 ActionResolver actionResolver = curTemplate.getAction((String)curAction.get(com.arcussmarthome.messages.type.Action.ATTR_TEMPLATE));
	        	 ActionTemplate actionTemplate = actionResolver.resolve(this.context);
	        	 curAction.put(com.arcussmarthome.messages.type.Action.ATTR_PREMIUM, Boolean.TRUE.equals(actionTemplate.getPremium())) ;
	         }	             
         }
