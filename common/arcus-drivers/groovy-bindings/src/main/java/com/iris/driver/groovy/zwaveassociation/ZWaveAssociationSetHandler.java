...
/**
 * Handles zwda:SetAssociation requests.
 * Sends Association Set (0x85/0x01) then Association Get for readback.
 */
public class ZWaveAssociationSetHandler implements ContextualEventHandler<Object> {
   private static final Logger log = LoggerFactory.getLogger(ZWaveAssociationSetHandler.class);
   ...
   public boolean handleEvent(DeviceDriverContext context, Object event) throws Exception {
      ...
      Integer group = ZWaveDirectAssociationCapability.SetAssociationRequest.getGroup(request);
      Integer node = ZWaveDirectAssociationCapability.SetAssociationRequest.getNode(request);
      if (group == null || node == null) {
         context.respondToPlatform(ZWaveDirectAssociationCapability.SetAssociationResponse.builder()
               .withStatus("INVALID_PARAM")
               .build());
         return true;
      }

      log.debug("SetAssociation group={} node={}", group, node);

      // Send Association Set
      ZWaveCommand cmd = ZWaveAssociationUtil.buildCommand(ASSOCIATION_CC, "set",
