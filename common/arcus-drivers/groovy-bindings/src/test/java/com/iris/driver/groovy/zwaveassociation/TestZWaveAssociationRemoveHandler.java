...
public class TestZWaveAssociationRemoveHandler {
   ...
   private ZWaveAssociationRemoveHandler handler;

   @Before
   public void setUp() {
      handler = new ZWaveAssociationRemoveHandler();
      context = createNiceMock(DeviceDriverContext.class);
   }

   @Test
   public void testRejectsNullGroup() throws Exception {
      MessageBody request = ZWaveDirectAssociationCapability.RemoveAssociationRequest.builder()
            .withGroup(null)
            .withNode(5)
            .build();
      ...
      assertEquals("INVALID_PARAM",
            ZWaveDirectAssociationCapability.RemoveAssociationResponse.getStatus(responseCapture.getValue()));
      verify(context);
   }
   ...
   @Test
   public void testSendsAssociationRemoveAndGet() throws Exception {
      MessageBody request = ZWaveDirectAssociationCapability.RemoveAssociationRequest.builder()
            .withGroup(2)
            .withNode(5)
            .build();
      ...
      // Verify the Association Remove command bytes: [CC, CMD, group, node]
      ...
      assertEquals(4, removeBytes.length);
      ...
      assertEquals((byte) 2, removeBytes[2]);  // group
      assertEquals((byte) 5, removeBytes[3]);  // node
