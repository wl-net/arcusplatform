...
public class TestZWaveAssociationGetHandler {
   ...
   private ZWaveAssociationGetHandler handler;

   @Before
   public void setUp() {
      handler = new ZWaveAssociationGetHandler();
      context = createNiceMock(DeviceDriverContext.class);
   }

   @Test
   public void testRejectsNullGroup() throws Exception {
      MessageBody request = ZWaveDirectAssociationCapability.GetAssociationRequest.builder()
            .withGroup(null)
            .build();
      ...
      assertEquals("INVALID_PARAM",
            ZWaveDirectAssociationCapability.GetAssociationResponse.getStatus(responseCapture.getValue()));
      verify(context);
   }

   @Test
   public void testSendsAssociationGet() throws Exception {
      MessageBody request = ZWaveDirectAssociationCapability.GetAssociationRequest.builder()
            .withGroup(4)
            .build();
      ...
      assertEquals((byte) 4, getBytes[2]);  // group
