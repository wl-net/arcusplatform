...
public class ZigbeeLocalProcessingDefault implements ZigbeeLocalProcessing {
   ...
   public Observable<Boolean> zcl(long eui64, short profile, byte endpoint, short cluster,
      ...
      return Observable.create(sub -> {
         try {
            logger.trace("Sending ZCL command to {} cluster {} endpoint {}", Long.toHexString(eui64), cluster, endpoint);
            sub.onNext(true);
            sub.onCompleted();
         } catch (Exception ex) {
            sub.onError(ex);
   ...
   public Observable<Boolean> zclmsp(long eui64, int manuf, short profile, short endpoint, short cluster,
      ...
      return Observable.create(sub -> {
         try {
            logger.trace("Sending manufacturer-specific ZCL to {} manuf={} cluster={} cmd={}",
                  Long.toHexString(eui64), manuf, cluster, cmd);
            sub.onNext(true);
            sub.onCompleted();
         } catch (Exception ex) {
            sub.onError(ex);
