...
public class ZBMessageTranslator {
    ...
    * Sets mode=NORMAL (0x00), flags=CLEAR_HNF (0x01) to activate PIR reporting.
    */
   ...
   /**
    * Handles a ZDP Node Descriptor Response (cluster 0x8002) to populate
    * node descriptor fields for newly joined devices.
    */
   public static void handleNodeDescriptorResponse(com.zsmartsystems.zigbee.aps.ZigBeeApsFrame apsFrame) {
      int[] payload = apsFrame.getPayload();
      if (payload == null || payload.length < 5) {
         return;
      }

      // ZDP frame: [seqNum(1), status(1), nwkAddr(2), nodeDescriptor(13)]
      int status = payload[1] & 0xFF;
      if (status != 0x00) {
         return; // Not SUCCESS
      }

      int nwkAddr = (payload[2] & 0xFF) | ((payload[3] & 0xFF) << 8);
      ZBNetwork network = ZBServices.INSTANCE.getNetwork();
      ZBNode node = network.getNodeByNwk(nwkAddr);
      if (node == null) {
         return;
      }

      if (payload.length >= 17) {
         // Node descriptor is 13 bytes starting at offset 4
         // Byte 0-1: node flags (logical type, freq band, etc)
         // Byte 2: MAC capability flags
         // Byte 3-4: manufacturer code (little-endian)
         // Byte 5: max buffer size
         // Byte 6-7: max incoming transfer size (little-endian)
         // Byte 8-9: server mask (little-endian)
         // Byte 10-11: max outgoing transfer size (little-endian)
         // Byte 12: descriptor capability

         int nodeFlags = (payload[4] & 0xFF) | ((payload[5] & 0xFF) << 8);
         int macCap = payload[6] & 0xFF;
         int mfrCode = (payload[7] & 0xFF) | ((payload[8] & 0xFF) << 8);
         int maxBuf = payload[9] & 0xFF;
         int maxIncoming = (payload[10] & 0xFF) | ((payload[11] & 0xFF) << 8);
         int serverMask = (payload[12] & 0xFF) | ((payload[13] & 0xFF) << 8);
         ...
         logger.debug("Node descriptor for NWK={}: mfr=0x{}, maxBuf={}, macCap=0x{}, nodeFlags=0x{}",
               ...
               String.format("%04X", nodeFlags));

         // Update the existing node in-place to preserve vendor/model/online/etc
         node.setNodeFlags(nodeFlags);
         node.setMacCapabilityFlags(macCap);
         node.setManufacturerCode(mfrCode);
         node.setMaximumBufferSize(maxBuf);
         node.setMaximumIncomingTransferSize(maxIncoming);
         node.setMaximumOutgoingTransferSize(maxOutgoing);
         node.setServerMask(serverMask);
         node.setDescriptorCapability(descCap);
         network.saveNode(node);

         // Notify bootstrapper that descriptor is available for pending adds
         com.iris.agent.zigbee.process.ZBBootstrapper.onNodeDescriptorReceived(node.getIeeeAddr());
      }
   }
      ...
      if (apsFrame.getCluster() == 0x0500) {
         ...
         if (rawPayload != null && rawPayload.length > 3) {
            ...
            if (isClusterSpecific && cmdIdx < rawPayload.length && (rawPayload[cmdIdx] & 0xFF) == 0x01) {
               ...
               // For AlertMe devices, also send ModeChange(NORMAL) to activate PIR
