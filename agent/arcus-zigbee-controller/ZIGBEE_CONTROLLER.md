...
[zsmartsystems](https://github.com/zsmartsystems/com.zsmartsystems.zigbee) library (v1.2.4) as the
...
- `ZigbeeController` connects to the hub router as a `PortHandler` on address `ZBIG`
...
- **Protocol address**: Uses EUI-64 as 8-byte `ProtocolDeviceId`, matching hub bridge
  address `ZBIG`.
- **v1.2.4 API specifics**: `IeeeAddress` has no `toLong()` — manual conversion from
  `int[] getValue()`. `ZclFieldSerializer.getPayload()` returns `int[]` not `byte[]`.
  `NodeDescriptor` uses `getIncomingTransferSize()` not `getMaximumIncomingTransferSize()`.
